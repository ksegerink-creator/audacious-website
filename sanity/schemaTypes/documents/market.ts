...
export const market = defineType({
  ...
  fields: [
    defineField({name: 'title', title: 'Naam van de markt', type: 'string', validation: Rule => Rule.required(), group: 'basis'}),
    defineField({name: 'slug', title: 'URL van de pagina', type: 'slug', options: {source: 'title'}, validation: Rule => Rule.required(), group: 'beheer'}),
    defineField({name: 'order', title: 'Volgorde in overzichten', type: 'number', group: 'beheer'}),
    defineField({name: 'intro', title: 'Intro onder de grote titel', type: 'text', rows: 4, group: 'basis'}),
    defineField({name: 'image', title: 'Afbeelding bovenaan', type: 'image', options: {hotspot: true}, group: 'basis'}),
    defineField({name: 'hero', title: 'Uitgebreide bovenkant pagina', type: 'hero', group: 'basis'}),
    defineField({name: 'heroPanel', title: 'Rechter informatieblok bovenaan', type: 'object', group: 'basis', fields: [
    ...
    defineField({name: 'relatedServices', title: 'Gerelateerde werkzaamheden', type: 'array', group: 'content', of: [defineArrayMember({type: 'reference', to: [{type: 'service'}]})]}),
