...
function audaciousCurrentSlugCandidates() {
  const pathname = window.location.pathname.replace(/\/$/, '') || '/';
  if (pathname === '/' || pathname.endsWith('/index.html')) return ['index'];
  const file = pathname.split('/').pop() || '';
  const slug = file.replace(/\.html$/, '');
  const candidates = [slug];
  if (pathname.startsWith('/projecten/') && !slug.startsWith('project-')) candidates.push(`project-${slug}`);
  if (pathname.startsWith('/nieuws/') && !slug.startsWith('nieuws-')) candidates.push(`nieuws-${slug}`);
...
function audaciousResolveCtaHref(cta = {}) {
  ...
  if (!slug) return '';
  if (target._type === 'service' || target._type === 'market') return `/${slug}`;
  if (target._type === 'blogPost') return `/nieuws/${slug.replace(/^nieuws-/, '')}`;
  if (target._type === 'page' && slug.startsWith('project-')) return `/projecten/${slug.replace(/^project-/, '')}`;
  if (slug === 'index') return '/';
  if (slug === 'markten') return '/markten-en-diensten';
  return `/${slug}`;
}
...
function audaciousApplyHero(doc, type) {
  ...
  const eyebrow = hero.eyebrow || doc.categoryTitle || (type === 'service' ? 'Werkzaamheden' : type === 'market' ? 'Markt' : type === 'news' ? 'Nieuws' : 'Pagina');
...
function audaciousRenderBlocks(blocks = []) {
  ...
  main.querySelectorAll('.page-section, .cms-rendered-section').forEach((section) => section.remove());
  const html = blocks.map(audaciousRenderBlock).join('');
  if (html.trim()) hero.insertAdjacentHTML('afterend', html);
  return true;
}
...
function audaciousApplyFooter(settings) {
  ...
  if (email) document.querySelectorAll('a[href^="mailto:"]').forEach((link) => { if (link.textContent.includes('@') || link.href.includes('info@audacious.com')) { link.textContent = email; link.href = `mailto:${email}`; } });
  if (phone) document.querySelectorAll('a[href^="tel:"]').forEach((link) => { link.textContent = phone; link.href = `tel:${phone.replace(/[^+0-9]/g, '')}`; });
}

async function audaciousFetchPageContent(slugs) {
  const blockFields = `
...
async function audaciousInitPageImages() {
  ...
  try {
    const slugs = audaciousCurrentSlugCandidates();
    const result = await audaciousFetchPageContent(slugs);
...
[900, 1800, 3200, 4600].forEach((delay) => window.setTimeout(audaciousInitPageImages, delay));
